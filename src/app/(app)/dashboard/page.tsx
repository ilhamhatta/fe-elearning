...
type CoursesReportResponse =
  | CourseStat[]
  | { data?: CourseStat[]; total?: number };
...
type Course = {
  id: number;
  name?: string; // umum
  title?: string; // kalau backend pakai title
  code?: string;
  students_count?: number;
};
type CoursesIndexResponse = Course[] | { data?: Course[] };
...
export default async function DashboardPage() {
  ...
  // ----- Reports: Courses
  let rawCoursesReport: CoursesReportResponse | null = null;
  try {
    rawCoursesReport = await (
      await serverFetch("/reports/courses", { method: "GET" })
    ).json();
  } catch {}

  let coursesReportData: CourseStat[] = [];
  let coursesTotalFromObj: number | undefined;

  if (Array.isArray(rawCoursesReport)) {
    coursesReportData = rawCoursesReport;
  } else if (rawCoursesReport && typeof rawCoursesReport === "object") {
    if (Array.isArray(rawCoursesReport.data))
      coursesReportData = rawCoursesReport.data;
    if (typeof rawCoursesReport.total === "number")
      coursesTotalFromObj = rawCoursesReport.total;
  }
  ...
  // ----- Courses index (daftar kursus user/semua)
  let rawCoursesIndex: CoursesIndexResponse | null = null;
  try {
    rawCoursesIndex = await (
      await serverFetch("/courses", { method: "GET" })
    ).json();
  } catch {}

  let myCourses: Course[] = [];
  if (Array.isArray(rawCoursesIndex)) {
    myCourses = rawCoursesIndex;
  } else if (
    rawCoursesIndex &&
    typeof rawCoursesIndex === "object" &&
    Array.isArray(rawCoursesIndex.data)
  ) {
    myCourses = rawCoursesIndex.data;
  }
  ...
  const topCourses = coursesReportData
    .map((c) => ({
      name: c.name || c.course || "Tanpa nama",
      count: Number(c.students ?? c.students_count ?? c.total ?? 0),
    }))
  ...
  // top 5 kursus dari /courses
  const myCoursesTop = myCourses
    .map((c) => ({
      id: c.id,
      name: c.name || c.title || "Tanpa nama",
      code: c.code,
      count:
        typeof c.students_count === "number" ? c.students_count : undefined,
    }))
    .slice(0, 5);
